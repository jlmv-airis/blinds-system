...
class FPDFMR
{
    ...
    /**
     * Normaliza un arreglo de registros, agregando valores por defecto
     * para campos opcionales que pueden no venir del stored procedure.
     * Esto evita errores "Undefined array key" sin modificar la base de datos.
     */
    private function normalizeRows(&$rows, $defaults = [])
    {
        $commonDefaults = [
            'lot' => '',
            'cloth_idt' => 0,
            ...
            'corbatin_id' => 0,
            'article_riel' => '',
            'width_discount_riel' => 0,
            'article_velcro' => '',
            'width_discount_velcro' => 0,
            'article_base' => '',
            'width_discount_base' => 0,
            ...
            'if_chain_height' => 0,
            'perfil_priv' => null,
            'perfil_priv_width_discount' => null,
            'tube' => '',
            'tube_id' => null,
            'perfil' => '',
            'counterweight' => '',
    ...
    public function createTubeFile($materialRequestID,$production_date,$detailOrders,$lambrequinData,$corbatinData,$fijoData,$perfilPrivData,$detailClothsOrders,$type)
    {
        $this->normalizeRows($detailOrders);
        $this->normalizeRows($lambrequinData);
        $this->normalizeRows($corbatinData);
        $this->normalizeRows($fijoData);
        $this->normalizeRows($perfilPrivData);
        $this->normalizeRows($detailClothsOrders);
        $nameFile = '';
        switch ((INT)$type) {
            case 1: // FILE
                $itemStop = 0;
                ...
                // PERFIL PRIV
                if(COUNT($perfilPrivData) > 0 ) {
                    $itemStop = 0;
                    $itemsPerPage = 25;
                    if(count($perfilPrivData) < $itemsPerPage ) { $itemStop = count($perfilPrivData); } else { $itemStop = $itemsPerPage; }
                    $initReg = 0;
                    $totalPages = ceil(count($perfilPrivData) / $itemsPerPage);
                    for ($i = 1; $i <= $totalPages; $i++) {
                        $this->ppFileLandscapeHeader('Hoja corte Perfil Privacidad',40,$materialRequestID,$production_date);
                        // ITEMS
                        $x = 28;
                        while ( $initReg < $itemStop ) {
                            $this->pdf->SetFont("Arial", "", 9);
                            $this->pdf->SetXY(2, $x);
                            $this->pdf->Cell(23, 6, $perfilPrivData[$initReg]['nomen'].' '.$perfilPrivData[$initReg]['order_id'], 1, 1, "C", true);
                            $this->pdf->SetXY(24, $x);
                            $this->pdf->Cell(14, 6, $perfilPrivData[$initReg]['item_id'], 1, 1, "C", true);
                            $this->pdf->SetXY(38, $x);
                            $this->pdf->Cell(15, 6, $perfilPrivData[$initReg]['width'], 1, 1, "C", true);
                            $this->pdf->SetXY(53, $x);
                            $this->pdf->Cell(15, 6, $perfilPrivData[$initReg]['height'], 1, 1, "C", true);
                            $this->pdf->SetXY(68, $x);
                            $this->pdf->Cell(30, 6, utf8_decode($perfilPrivData[$initReg]['product']), 1, 1, "C", true);
                            $this->pdf->SetXY(98, $x);
                            $this->pdf->Cell(40, 6, $perfilPrivData[$initReg]['color_name'], 1, 1, "C", true);
                            $this->pdf->SetFont("Arial", "", 6);
                            $this->pdf->SetXY(138, $x);
                            $this->pdf->Cell(78, 6, $perfilPrivData[$initReg]['article_pp'].' '.$perfilPrivData[$initReg]['quantity_pp'].' PIEZAS DE '.$perfilPrivData[$initReg]['width_pp'] , 1, 1, "C", true);
                            $this->pdf->SetXY(216, $x);
                            $this->pdf->Cell(78, 6, $perfilPrivData[$initReg]['article_felpa'].' '.$perfilPrivData[$initReg]['quantity_pp'].' PIEZAS DE '.$perfilPrivData[$initReg]['width_pp'] , 1, 1, "C", true);
                            $x = $x + 6;
                            $initReg++;
                        }
                        $itemStop = $itemStop + $itemsPerPage;
                        $this->pdf->SetFont("Arial", "", 12);
                        if($itemStop > count($perfilPrivData) ) { $itemStop = count($perfilPrivData) ; }
                        $this->pdf->Text(260, 205, utf8_decode("Página ".$i." de ".$totalPages));
            ...
            case 2: // LABELS
                ...
                foreach ($detailOrders as $key => $do) {
                    ...
                    // perfil priv
                    if(!is_null($do['perfil_priv'])) {
                        $this->pdf->SetFont("Arial", "", 8);
                        $this->pdf->SetXY(1, $y);
                        $this->pdf->Cell(50, 4, utf8_decode(ucfirst(strtolower($do['perfil_priv']))), 0, 1, "L", false);
                        $this->pdf->SetXY(50, $y);
                        $this->pdf->Cell(5, 4, '2 de:', 0, 1, "C", false);
                        $this->pdf->SetFont("Arial", "", 8);
                        $this->pdf->SetXY(56, $y);
                        $this->pdf->Cell(20, 4,  number_format($do['perfil_priv_width_discount'],3), 0, 1, "C", false);
                        $y = $y + 4;
                    }
                    //
                    $this->pdf->SetFont("Arial", "", 8);
    ...
    //                     $this->pdf->setFillColor(241,241,241);
    //                     $this->pdf->SetXY(2, $x);
    //                     $this->pdf->Cell(293, 6, utf8_decode(' '.$this->getFullComment($orders[$initReg],$orders),1), 1, 1, "L", true);
    //                     $this->pdf->setFillColor(255,255,255);
    //                     $x = $x + 6;
    //                     $initReg++;
    //                 }
    ...
    public function createclothFile($materialRequestID,$production_date,$orders,$detailOrders,$graficOrders,$type)
    {
        $this->normalizeRows($orders);
        $this->normalizeRows($detailOrders);
        $this->normalizeRows($graficOrders);
        $nameFile = '';
        switch ((INT)$type) {
            case 1: // FILE
                // dd($graficOrders);
                ...
                foreach ($graficOrders as $cut) {
                    if( $cut['width_lot'] === 3 ) { $pixelZise = 0.23333; }
                    if( $cut['width_lot'] == 2.9 ) { $pixelZise = 0.24111; }
                    if( $cut['width_lot'] == 2.8 ) { $pixelZise = 0.24001; }
                    if( $cut['width_lot'] == 2.5 ) { $pixelZise = 0.27999; }
                    if( $cut['width_lot'] == 2 ) { $pixelZise = 0.34888; }
                    if( $cut['width_lot'] == 2.99 ) { $pixelZise = 0.23333; }
                ...
                foreach ($detailOrders as $key => $do) {
                    if( (INT)$do['product_id'] === 1 OR (INT)$do['product_id'] === 2 ) {
                        ...
                        if( (INT)$do['cloth_idt'] === 1 OR  (INT)$do['damage_fabric'] === 1 ) {
                            ...
                            $this->pdf->SetXY(1, 20);
                            $ifChainHeight = '';
                            if( (INT)$do['if_chain_height'] === 1 && (INT)$do['mechanism_id'] === 3 ) {
                                $ifChainHeight = ', ALTO CADENA A '.$do['height'];
                            }
                            $this->pdf->MultiCell(70, 4, utf8_decode('obs. '.$this->getComment($do,$detailOrders).$ifChainHeight), 0, "L", false);
                        }
                        // // OLD
                        // $this->pdf->AddPage();
                        // $this->pdf->SetXY(1, 1);
    ...
    {
        $this->normalizeRows($detailOrdersLambre);
        $this->normalizeRows($detailOrdersCorba);
        $this->normalizeRows($detailOrdersFijo);
        $nameFile = '';
        switch ((INT)$type) {
            case 1: // FILE
    ...
    public function createAccesoriesFile($materialRequestID,$production_date,$detailOrders,$detailClothsOrders,$type)
    {
        $this->normalizeRows($detailOrders);
        $this->normalizeRows($detailClothsOrders);
        $nameFile = '';
        switch ((INT)$type) {
            ...
            case 2: // LABELS
                ...
                $this->pdf->SetAutoPageBreak(true,2);
                foreach ($detailOrders as $key => $do) {
                    if( (INT)$do['side_id'] === 1 OR (INT)$do['product_id'] === 4) {
                        $y = 10;
                        $this->pdf->AddPage();
                        $this->pdf->SetFont("Arial", "B", 9);
                        $this->pdf->SetXY(1, 4);
                        ...
                        foreach ($do['items_acc'] as $item) {
                            ...
                            $this->pdf->SetFont("Arial", "B", 8);
                            $this->pdf->SetXY(56, $y);
                            if( (INT)$item['from_data'] === 7 ) { $this->pdf->Cell(20, 4, '2', 0, 1, "C", false); }
                            else { $this->pdf->Cell(20, 4, $item['quantity'], 0, 1, "C", false); }
                            $y = $y + 4;
                        }
                ...
                foreach ($detailOrders as $key => $do) {
                    ...
                    if( (INT)$do['operation_id'] === 1 ) {
                        if( (INT)$do['product_id'] === 1 ) {
                            ...
                            $this->pdf->SetFont("Arial", "", 8);
                            $this->pdf->SetXY(56, $y);
                            $this->pdf->Cell(20, 4, $this->getchainSize($heightFinalChain,$heightFinal,$do['mechanism_id'],$do['relation_cassette'],$do['tube_id'],$do['product_id'],$do['if_chain_height']), 0, 1, "C", false);
                            $y = $y + 4;
                        }
                        if( (INT)$do['product_id'] === 2 ) {
                            ...
                            $this->pdf->SetFont("Arial", "", 8);
                            $this->pdf->SetXY(56, $y);
                            $this->pdf->Cell(20, 4, $this->getchainSize($do['height_chain'],$do['height'],$do['mechanism_id'],$do['relation_cassette'],$do['tube_id'],$do['product_id'],0), 0, 1, "C", false);
                            $y = $y + 4;
                            // Contrapeso  cadena
                            $this->pdf->SetFont("Arial", "", 8);
                            $this->pdf->SetXY(1, $y);
                    ...
                    // TENSOR
                    if($do['relation_tensor'] > 0 AND  (INT)$do['product_id'] === 1 ) {
                        $this->pdf->SetFont("Arial", "", 8);
                        $this->pdf->SetXY(1, $y);
                        $this->pdf->Cell(50, 4, utf8_decode(ucfirst(strtolower('TENSOR CADENA CLEAR'))), 0, 1, "L", false);
                        $this->pdf->SetFont("Arial", "", 8);
    ...
    public function createPackagingModulation($materialRequestID,$production_date,$orders,$detailOrders,$type)
    {
        ...
        foreach ($orders as $key => $order) {
            if (isset($order['details']) && is_array($order['details'])) {
                $this->normalizeRows($orders[$key]['details']);
            }
        }
        $nameFile = '';
        switch ((INT)$type) {
            case 1: // FILE
                $this->pdf->SetAutoPageBreak(true,2);
                $this->pdf->AddPage();
                $this->pdf->SetFont("Arial", "B", 120);
                $this->pdf->SetXY(1, 1);
                $this->pdf->Cell(75, 46, 'Z1', 0, 1, "C", false);
                $this->pdf->Code128(17, 50, 'Z1', 44, 20);
                $nameFile = "ubicaciones-empaque-";
            ...
            case 2: // LABELS
                ...
                foreach ($orders as $key => $order) {
                    ...
                    foreach ($order['details'] as $key => $do) {
                        ...
                        $this->pdf->SetXY(1, 1);
                        $detailOrder = $order['nomen'].$order['id'].' - '.$do['item_id'];
                        if( $order['nomen'] == 'GLS' OR $order['nomen'] == 'SLS' ) {
                            $detailOrder = $order['nomen'].$order['folio'].' - '.$do['item_id'];
                        }
                        $this->pdf->Cell(45, 15, $detailOrder, 0, 1, "L", false);
                        // created at
                        ...
                        } else if($order['nomen'] == 'SLS') {
                            $this->pdf->Code128(17, 60, 'SLS '.$do['id'], 44, 10);
                            $this->pdf->SetFont("Arial", "", 6);
                            $this->pdf->SetXY(17, 70);
                            $this->pdf->Cell(44, 3, 'SLS '.$do['id'], 0, 1, "C", false);
                        } else {
    ...
    public function createChecklistfile($materialRequestID,$production_date,$orders,$type) {
        // dd($orders);
        $this->normalizeRows($orders);
        foreach ($orders as $key => $order) {
            if (isset($order['details']) && is_array($order['details'])) {
                ...
                foreach ($orders as $key => $order) {
                    ...
                    foreach ($order['details'] as $key => $do) {
                        if( (INT)$do['product_id'] === 1 OR (INT)$do['product_id'] === 2 ) {
                            ...
                            $this->pdf->SetXY(1, 2);
                            $this->pdf->MultiCell(75, 6, 'CHECKLIST FINAL', 0, "C", false);

                            $this->pdf->SetXY(1, 8);
                            $this->pdf->Cell(38, 6, $do['order_id'].' - '.$do['item_id'], 0, 1, "C", false);
                            $this->pdf->SetFont("Arial", "", 10);
                            $this->pdf->SetXY(39, 8);
                            $this->pdf->Cell(37, 6, Carbon::createFromFormat('Y-m-d H:i:s', $do['created_at'])->format('d/m/Y'), 0, 1, "C", false);
                            //
                            $y = 18;
                            $this->pdf->SetFont("Arial", "", 6);
                            // TUBO
                            if(!is_null($do['tube']) OR $do['tube'] != '' ) {
                                $this->pdf->SetXY(2, $y);
                                $this->pdf->Cell(52, 4, $do['tube'], 0, 1, "L", false);
                                $this->pdf->SetXY(54, $y);
                                $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                                $y = $y + 5;
                            }
                            // CONTRAPESO
                            if(!is_null($do['counterweight_bar']) OR $do['counterweight_bar'] != '' ) {
                                $this->pdf->SetXY(2, $y);
                                $textBase = 'Base '.$do['counterweight_bar'].' '.$do['color_name'];
                                if( (INT)$do['counterweight_bar_id'] === 4 ) { $textBase = 'Base '.$do['counterweight_bar']; }
                                $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper($textBase))), 0, 1, "L", false);
                                $this->pdf->SetXY(54, $y);
                                $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                                $y = $y + 5;
                            }
                            // FASCIA
                            if( (INT)$do['relation_cassette'] > 0 ) {
                                $this->pdf->SetXY(2, $y);
                                $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('Fascia '.$do['color_name']))), 0, 1, "L", false);
                                $this->pdf->SetXY(54, $y);
                                $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                                $y = $y + 5;
                            }
                            ...
                            $this->pdf->SetXY(2, $y);
                            if( (INT)$do['operation_id'] === 1 ) {
                                $this->pdf->Cell(52, 4, utf8_decode(strtoupper('OperaciÓn Manual ')), 0, 1, "L", false);
                            } else {
                                $this->pdf->Cell(52, 4, utf8_decode(strtoupper('OperaciÓn Motorizada ('.$do['type_motor_desk'].')')), 0, 1, "L", false);
                            }
                            $this->pdf->SetXY(54, $y);
                            $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                            $y = $y + 5;
                            // LADO MECANISMO
                            $this->pdf->SetXY(2, $y);
                            $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('LADO DE CONTROL '.$do['mechanism_side']))), 0, 1, "L", false);
                            $this->pdf->SetXY(54, $y);
                            $this->pdf->SetXY(54, $y);
                            $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                            $y = $y + 5;
                            ...
                            if( (INT)$do['operation_id'] === 1 ) {
                                $this->pdf->SetXY(2, $y);
                                if( (INT)$do['chain_id'] === 1 ) {
                                    $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('Cadena '.$do['chain'].' '.$do['color_name']))), 0, 1, "L", false);
                                } else {
                                    $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('Cadena '.$do['chain']))), 0, 1, "L", false);
                                }
                                $this->pdf->SetXY(54, $y);
                                $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                                $y = $y + 5;
                            ...
                            // CAIDA
                            $this->pdf->SetXY(2, $y);
                            $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('Caída '.$do['fall']))), 0, 1, "L", false);
                            $this->pdf->SetXY(54, $y);
                            $this->pdf->Cell(18, 4, '', 1, 1, "L", false);
                            $y = $y + 5;
                            // OBSERVACIONES
                            $this->pdf->SetXY(2, $y);
                            $this->pdf->Cell(52, 4, utf8_decode(ucfirst(strtoupper('Observaciones: '))), 0, 1, "L", false);
                            $y = $y + 5;
                            $this->pdf->SetXY(2, $y);
                            $ifChainHeight = '';
                            if( (INT)$do['if_chain_height'] === 1 && (INT)$do['mechanism_id'] === 3 ) {
                                $ifChainHeight = ', CADENA A LO ALTO';
                            }
                            $this->pdf->MultiCell(73, 4, $this->getFullComment($do,$order['details'],2).$ifChainHeight, 0, "L", false);
                            $y = $y + 12;
                            // Reviso
                            $this->pdf->SetXY(2, 71);
                            $this->pdf->Cell(74, 4, utf8_decode(ucfirst(strtoupper('REVISO '))), 'T', 1, "C", false);
    ...
    private function getFullComment($item,$cuts,$opt) {
        $textDetail = '';
        switch ((INT)$item['product_id']) {
            case 1:
                if((INT)$item['is_inverted'] === 1) { $textDetail .= 'Rotar tela, '; }
                if((INT)$item['is_heat_seal'] === 1) { $textDetail .= 'Termosellada, '; }
                if((INT)$item['relation_cassette'] > 0 && (INT)$opt !== 2) { if((INT)$item['divisions'] > 1 ) { if((INT)$item['relation_cassette'] > 0) { $textDetail .= $this->isCasseteDetail($item,$cuts); } } else { $textDetail .= 'Con cassette, '; } }
                if((INT)$item['relation_lambrequin'] > 0 && (INT)$opt !== 2) { $textDetail .= 'Con Lambrequin, '; }
                if((INT)$item['relation_bracket'] > 0 && (INT)$opt !== 2) { $textDetail .= $this->isBracketRelation($item,$cuts); }
                ...
                if((INT)$item['mechanism_id'] > 0  && (INT)$opt === 1 ) { $textDetail .= $item['mechanism'].', '; }
                if((INT)$item['mechanism_id'] > 0 && (INT)$opt === 2 ) { $textDetail .= 'MECANISMO '.$item['mechanism'].', '; }
                if( (INT)$opt !== 2 ) { $textDetail .= $item['tube'].', '; }
            break;
            case 2:
                if((INT)$item['divisions'] > 1  && (INT)$opt !== 2 ) { $textDetail .= $this->isCasseteDetail($item,$cuts); }
                if((INT)$item['relation_bracket_dn'] > 0 ) { $textDetail .= 'En Soporte dia y noche, '; }
                if((INT)$item['mechanism_id'] > 0  && (INT)$opt === 1 ) { $textDetail .= $item['mechanism'].', '; }
                if((INT)$item['mechanism_id'] > 0 && (INT)$opt === 2 ) { $textDetail .= 'MECANISMO '.$item['mechanism'].', '; }
                if( (INT)$opt !== 2 ) { $textDetail .= $item['tube'].', '; }
    ...
    private function ppFileLandscapeHeader($section,$x,$materialRequestID,$production_date) {
        ...
        $this->pdf->Cell(40, 8, utf8_decode('Color') , 1, 1, "C", true);
        $this->pdf->SetXY(138, 20);
        $this->pdf->Cell(78, 8, 'Perfil privacidad' , 1, 1, "C", true);
        $this->pdf->SetXY(216, 20);
        $this->pdf->Cell(78, 8, 'Felpa' , 1, 1, "C", true);
        $this->pdf->setFillColor(255, 255, 255);
    }
