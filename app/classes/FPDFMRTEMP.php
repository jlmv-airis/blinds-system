...
class FPDFMR
{
            ...
            case 1: // FILE
                ...
                for ($i = 1; $i <= $totalPages; $i++) {
                    ...
                    while ( $initReg < $itemStop ) {
                        ...
                        $this->pdf->SetXY(2, $x);
                        $this->pdf->Cell(23, 6, $detailOrders[$initReg]['order_id'], 1, 1, "C", true);
                        $this->pdf->SetXY(24, $x);
                ...
                if(COUNT($lambrequinData) > 0 ) {
                    ...
                    for ($i = 1; $i <= $totalPages; $i++) {
                        ...
                        while ( $initReg < $itemStop ) {
                            $this->pdf->SetFont("Arial", "", 9);
                            $this->pdf->SetXY(2, $x);
                            $this->pdf->Cell(23, 6, $lambrequinData[$initReg]['order_id'], 1, 1, "C", true);
                            $this->pdf->SetXY(24, $x);
                ...
                if(COUNT($corbatinData) > 0 ) {
                    ...
                    for ($i = 1; $i <= $totalPages; $i++) {
                        ...
                        while ( $initReg < $itemStop ) {
                            $this->pdf->SetFont("Arial", "", 9);
                            $this->pdf->SetXY(2, $x);
                            $this->pdf->Cell(23, 6, $corbatinData[$initReg]['order_id'], 1, 1, "C", true);
                            $this->pdf->SetXY(24, $x);
                ...
                if(COUNT($fijoData) > 0 ) {
                    ...
                    for ($i = 1; $i <= $totalPages; $i++) {
                        ...
                        while ( $initReg < $itemStop ) {
                            $this->pdf->SetFont("Arial", "", 9);
                            $this->pdf->SetXY(2, $x);
                            $this->pdf->Cell(23, 6, $fijoData[$initReg]['order_id'], 1, 1, "C", true);
                            $this->pdf->SetXY(24, $x);
    ...
    public function createclothFile($materialRequestID,$production_date,$detailOrders,$graficOrders,$type)
    {
        $nameFile = '';
        switch ((INT)$type) {
            case 1: // FILE
                ...
                foreach ($graficOrders as $cut) {
                    ...
                    if((INT)$cut['article_id'] !== (INT)$articleIdOld ) {
                        ...
                        $this->pdf->SetFont("Arial", "B", 8);
                        $this->pdf->SetXY($x, $y);
                        $this->pdf->Cell(70, 5, utf8_decode($cut['article']).' '.$widtLotHeader, 0, 1, "C", true);
                    ...
                    if(is_null($cut['join_id'])) {
                        ...
                        if((INT)$cut['is_corbatin'] === 1) { // NOT IS CORBATIN
                            ...
                            $this->pdf->TextWithRotation($x+2.5,$y+$height-2,'CORBATIN '.$cut['order_id']." - ".$cut['item_id'].' / '.$widthText." X ".$heightText ,90,0);
                            $this->pdf->SetFont("Arial", "", 7);
            ...
            case 2: // LABELS
                ...
                foreach ($orders as $key => $order) {
                    ...
                    foreach ($order['details'] as $key => $do) {
                        ...
                        $this->pdf->SetXY(1, 1);
                        $this->pdf->Cell(45, 15, $do['order_id'].' - '.$do['item_id'], 0, 1, "L", false);
                        // created at
