    ...
    public function downloadRequestFile($user_id,$file,$type,$materialRequestID) {
        ...
        switch ($file) {
            case 'tubes':
                ...
                // PERFIL PRIV - Deshabilitado: sp_modulation no tiene opción 16 definida
                $perfilPrivData = [];
                // DETAILS
                $detailClothsOrders = [];
                ...
                foreach ($detailOrders as $key => $do) {
                    ...
                    $detailOrders[$key]['perfil_priv'] = null;
                    $detailOrders[$key]['perfil_priv_width_discount'] = null;

                    if(COUNT( $tubeIDData) > 0 ) {
                    ...
                    if(COUNT($perfilPrivData) > 0) {
                        foreach ($perfilPrivData as $perfp) {
                            if($perfp['id'] === (INT)$do['id']) {
                                $detailOrders[$key]['perfil_priv'] = $perfp['article_pp'];
                                $detailOrders[$key]['perfil_priv_width_discount'] =  $perfp['height'];
                            }
                        }
                    }
                }
                switch ((INT)$type) {
                    case 1: // Files
                        ...
                        return $pdf->createTubeFile($materialRequestID,$materialRequest->production_date,$detailOrders,$lambrequinData,$corbatinData,$fijoData,$perfilPrivData,$detailClothsOrders,$type);
                    break;
                    case 2: // Stickers
            ...
            case 'accesories':
                ...
                switch ((INT)$type) {
                    case 1: // Files
                        return app(FPDFMR::class)->createAccesoriesFile($materialRequestID,$materialRequest->production_date,$detailOrders,$detailClothsOrders,$type);
                    break;
                    case 2: // Stickers
                        $pdf = new FPDFMR(new PDF_Code128('L','mm',[77,77]));
                        return $pdf->createAccesoriesFile($materialRequestID,$materialRequest->production_date,$detailOrders,$detailClothsOrders,$type);
                    break;
                }
            break;
            case 'mechanisms':
            ...
            case 'packaging':
                ...
                foreach ($orders as $key => $do) {
                    ...
                    foreach ($detailOrders as $detail) {
                        if( (INT)$detail['order_id'] == (INT)$do['id'] && $detail['nomen'] == $do['nomen'] ) {
            ...
            case 'cheklist':
                ...
                $detailClothsOrders = [];
                // Opción 17 no existe en sp_modulation; se usa opción 1 (detail orders) que contiene los campos necesarios para checklist
                $statementClothsOrders = DB::getPdo()->prepare("CALL sp_modulation(14,".$materialRequestID.",1,0,0,0.0,0.0,0.0,'','','','','')");
                $statementClothsOrders->execute();
                do {  $resultClothsOrders[] = $statementClothsOrders->fetchAll(\PDO::FETCH_OBJ); } while ($statementClothsOrders->nextRowSet());
                foreach (json_decode(json_encode($resultClothsOrders[0]), true)  as $value) { $detailOrders[] = $value; }
                //
                foreach ($orders as $key => $do) {
                    $orders[$key]['details'] = [];
                    foreach ($detailOrders as $detail) {
                        if( (INT)$detail['order_id'] == (INT)$do['id'] && (INT)$detail['nomen'] == (INT)$do['nomen'] ) {
                            $orders[$key]['details'][] = $detail;
                        }
                    }
                }
                switch ((INT)$type) {
                    case 1: // Files
                        $pdf = new FPDFMR(new PDF_Code128("L", "mm", "A4"));
                        return $pdf->createChecklistfile($materialRequestID,$materialRequest->production_date,$orders,$type);
                    break;
                    case 2: // Stickers
                        $pdf = new FPDFMR(new PDF_Code128('L','mm',[77,77]));
                        return $pdf->createChecklistfile($materialRequestID,$materialRequest->production_date,$orders,$type);
            ...
            case 'lambrequin':
                ...
                switch ((INT)$type) {
                    case 1: // Files
                        $pdf = new FPDFMR(new PDF_Code128("L", "mm", "A4"));
                        return $pdf->createpclothsgalFile($materialRequestID,$materialRequest->production_date,$detailOrdersLambre,[],[],$type);
                    break;
                    case 2: // Stickers
                        $pdf = new FPDFMR(new PDF_Code128('L','mm',[26,77]));
                        return $pdf->createpclothsgalFile($materialRequestID,$materialRequest->production_date,$detailOrdersLambre,[],[],$type);
